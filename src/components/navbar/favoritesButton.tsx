...
const FavoritesButton: React.FC = observer(() => {
	...
	// ↑ ЭТА СТРОЧКА — ВСЁ, ЧТО НУЖНО!

	// Типы выводятся автоматически из menuData — идеально!
	type MenuItem = typeof menuData[keyof typeof menuData][number];

	const MenuButton: React.FC<MenuItem> = ({ icon, onClick, text, color, strokeWidth, fill, viewBox }) => (
		<div>
	...
	const MenuSection: React.FC<{ title: string; items: readonly MenuItem[] }> = ({ title, items }) => (
