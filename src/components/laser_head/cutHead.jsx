...
export const CutHead = observer(({ keyInd }) => {

    const { technology, selectedPiercingStage, selectedPiercingMacro, animProgress, isAnimating,  isPaused , atEnd } = macrosStore;
    const bs = 175.0;
 	const macro= technology.piercingMacros[selectedPiercingMacro]['stages'];
  


 	const getValue = (param, stage = animProgress.stage, progress = animProgress.progress) => {
		...
		const currentTime = data[currentStage].power_W_s / data[currentStage].power  ||0;
	...
	useEffect(() => {
		...
		function startStageAnimation(initialProgress) {
	...
	const degeneratePath =() =>{
		let d  = "M 55 0 L 55 89.733 C 55 89.733 69.987 161.318 73 201.781 C 73.312 205.966 73 210.3 73 214.39 C 71.626 221.321 70 233.3 70 233.3 L 80 233.3 C 80 233.3 78.4 221.3 77 214.39 C 77 210.3 76.688 205.966 77 201.781 C 80.013 161.318 95 89.733 95 89.733 L 95 0 Z"
