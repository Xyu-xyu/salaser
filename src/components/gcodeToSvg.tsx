...
function parseGCodeToBlocks(gcode: string) {
	...
	let posX:number|null=0
	let posY:number|null=0
    ...
    for (let line of lines) {		
        if (line.includes('G0')) {
            ...
            if (newX != null && newY != null && currentX != null && currentY != null) {
                svg.push(`M${currentX} ${height - currentY} L${utils.round(newX)} ${utils.round(height - newY)} `);
                currentX = newX;
                currentY = newY;
            } else if (newX != null && currentX != null && currentY != null) {
                svg.push(`M${currentX} ${height - currentY} H${utils.round(newX)} `);
                currentX = newX;
            } else if (newY != null && currentX != null && currentY != null) {
                svg.push(`M${currentX} ${height - currentY} V${utils.round(height - newY)}`);
                currentY = newY;
            }
            path = 'continued';
            ...
            if (newX != null && newY != null && currentX != null && currentY != null) {
                ...
                let largeArcFlag = 0//utils.getLargeArcFlag(currentX, currentY, newX, newY, offsetI, offsetJ, 0);
                svg.push(`M${currentX} ${height - currentY} A${r} ${r} 0 ${ largeArcFlag } 1 ${utils.round(newX)} ${utils.round(height - newY)} `);
                currentX = newX;
                currentY = newY;
            }
            ...
            if (newX != null && newY != null && currentX != null && currentY != null) {
                let r = utils.round(Math.hypot( newX- offsetI,  newY-  offsetJ));
                let largeArcFlag = 0//utils.getLargeArcFlag(currentX, currentY, newX, newY, offsetI, offsetJ, 1);
                svg.push(`M${currentX} ${height - currentY} A${r} ${r} 0 ${ largeArcFlag } 0 ${utils.round(newX)} ${utils.round(height - newY)} `);
                currentX = newX;
                currentY = newY;
            }
        } 
